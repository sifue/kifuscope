...
## 2026-07-07 認識失敗時の配信表示保持

### 実装・文書化した内容

- 一度でも正常な評価値または確定局面を配信した後は、一時的な `recognition_failed` / `position_unconfirmed` をOBSオーバーレイへ表示しないようにした
- 認識失敗の詳細メッセージ、信頼度、現在追跡中SFENは `serve-realtime` のコンソールログへ出すようにした
- 新局面の評価中も、初回評価後は直前の評価値・最善手・深さを保持するようにした
- 起動直後でまだ正常表示がない場合だけ、従来通り認識中・認識失敗を表示する
- READMEに配信画面での認識失敗表示方針を追記した
